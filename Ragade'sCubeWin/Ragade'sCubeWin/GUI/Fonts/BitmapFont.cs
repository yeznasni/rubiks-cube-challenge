...
namespace RagadesCubeWin.GUI.Fonts
{
	/// <summary>
    ...
    public struct GlyphTextureInfo
    {
    ...
    public class GlyphTextureString
    {
        private List<GlyphTextureInfo> _listGlyphTextureInfo;
        private int _nWidth;

        public GlyphTextureString(
            List<GlyphTextureInfo> listGlyphTextureInfo,
            int nWidth)
        {
            _listGlyphTextureInfo = listGlyphTextureInfo;
            _nWidth = nWidth;
        }

        public List<GlyphTextureInfo> TexureInfoList
        {
            get { return _listGlyphTextureInfo; }
        }
	...
	public class BitmapFont
	{
        ...
        private Dictionary<int, Texture2D> m_dictGlyphID2TextureInfo;
		...
		{
            m_dictGlyphID2TextureInfo = new Dictionary<int, Texture2D>();
		...
		public void Reset(GraphicsDevice device, ContentManager content)
		{
			m_sb = new SpriteBatch(device);
            
			foreach (KeyValuePair<int, BitmapInfo> kv in m_dictBitmapID2BitmapInfo)
                m_dictBitmapID2Texture[kv.Key] = content.Load<Texture2D>(kv.Value.strFilename);

            m_dictGlyphID2TextureInfo.Clear();

            foreach (GlyphInfo ginfo in m_dictUnicode2GlyphInfo.Values)
            {
                if (ginfo.nWidth != 0 && ginfo.nHeight != 0)
                {
                    Rectangle r = new Rectangle(ginfo.nOriginX, ginfo.nOriginY, ginfo.nWidth, ginfo.nHeight);

                    Color[] pixels = new Color[ginfo.nWidth * ginfo.nHeight]; //same size that we wantbigTexture.GetData<Color>(1, sourceArea, pixels, 0, pixels.Length);

                    // Lift the pixels from the large glyph texture.
                    m_dictBitmapID2Texture[ginfo.nBitmapID].GetData<Color>(
                        0,
                        r,
                        pixels,
                        0,
                    ...
                    // Create new texture consitsting of only the glyph
                    ...
                    m_dictGlyphID2TextureInfo[ginfo.nGlyphID] = glyphTexture;
                }
            }
		}
        ...
        /// <param name="args">String format args</param>
        /// <returns>Width of string (in pixels)</returns>
        public int GenerateGlyphTextureString(string format, params object[] args)
        {
            return DrawString(m_vPen, m_color, format, args);
        }

        /// <summary>
        /// Generates a set of glyph textures contained in a GlyphTextureString object.
        /// </summary>
        /// <param name="vOrigin"></param>
        /// <param name="format"></param>
        /// <param name="args"></param>
        /// <returns></returns>
        public GlyphTextureString GenerateGlyphTexturesString(Vector2 vOrigin, string format, params object[] args)
        {
            
            string str = string.Format(format, args);

            List<GlyphTextureInfo> listTexureInfo = new List<GlyphTextureInfo>(str.Length);
            ...
            foreach (char c in str)
            {
                ...
                if (ginfo.nWidth != 0 && ginfo.nHeight != 0)
                {
                    GlyphTextureInfo textureInfo = new GlyphTextureInfo();
                    textureInfo.glyphTexture = m_dictGlyphID2TextureInfo[ginfo.nGlyphID];
                    textureInfo.position = new Point((int)vAt.X, (int)vAt.Y);
                    textureInfo.size = new Point(ginfo.nWidth, ginfo.nHeight);

                    listTexureInfo.Add(textureInfo);
                }
            ...
            return new GlyphTextureString(listTexureInfo, nWidth);
        }
